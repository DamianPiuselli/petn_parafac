...
def match_and_align_profiles(A_pred, B_pred, C_pred, A_true, B_true, C_true):
    ...
    B_pred = B_pred.astype(float).copy()
    C_pred = C_pred.astype(float).copy()
    A_true = A_true.astype(float).copy()
    B_true = B_true.astype(float).copy()
    ...
    B_pred_ordered = np.zeros((B_pred.shape[0], R_true))
    ...
    b_sims = [calculate_cosine_similarity(B_pred_ordered[:, r], B_true[:, r]) for r in range(R_true)]
    c_sims = [calculate_cosine_similarity(C_pred_ordered[:, r], C_true[:, r]) for r in range(R_true)]
    
    return {
        'a_ordered': A_pred_ordered,
        'b_ordered': B_pred_ordered,
...
def main():
    ...
    metrics_mcr = match_and_align_profiles(mcr.A_, mcr.B_, mcr.C_, A_true_scores, B_pure, C_true_spec)
    
    print("  Fitting COW-PARAFAC...")
    cow = COWPARAFAC(num_components=3, N_seg=8, slack=3)
    cow.fit(X_noisy_norm)
    metrics_cow = match_and_align_profiles(cow.A_, cow.B_, cow.C_, A_true_scores, B_pure, C_true_spec)
    ...
    metrics_petn_lin = match_and_align_profiles(A_petn_lin, B_petn_lin, C_petn_lin, A_true_scores, B_pure, C_true_spec)
    ...
    metrics_petn_spl = match_and_align_profiles(A_petn_spl, B_petn_spl, C_petn_spl, A_true_scores, B_pure, C_true_spec)
    ...
    print("Generating GC-MS performance plots...")
    ...
    # 1. Resolved profiles comparison (Linear model)
    plot_chroma_resolved_vs_true_profiles(
        B_pure, C_true_spec, metrics_petn_lin['b_ordered'], metrics_petn_lin['c_ordered'],
        t, np.linspace(200.0, 400.0, 150),
        save_path=os.path.join(plot_dir, 'simulated_resolved_profiles.png')
    )
    ...
    plot_chroma_alignment_comparison(
        t, X_noisy_norm, X_aligned,
        save_path=os.path.join(plot_dir, 'simulated_alignment_comparison.png')
    )
    
    # 3. Scores (concentrations) parity comparison plot
    plot_scores_parity(
        A_true_scores, metrics_petn_lin['a_ordered'],
        save_path=os.path.join(plot_dir, 'simulated_scores_quantification.png')
    )
