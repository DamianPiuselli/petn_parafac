...
def train_chroma_petn(dataset, epochs=1200, lr=0.01, warp_reg_coef=0.001, warp_type='linear',
    ...
    if X.ndim != 3:
        raise ValueError(f"Input data must be 3-dimensional (shape: I x J x K), got {X.ndim}D")
    ...
    if derivative_order > 0:
        ...
        X_deriv = savgol_filter(X, window_length=sg_window_size, polyorder=sg_polyorder, deriv=derivative_order, axis=1)
        y_target = torch.tensor(X_deriv, dtype=torch.float32)[coords_i, coords_j, coords_k]
    else:
        y_target = torch.tensor(X, dtype=torch.float32)[coords_i, coords_j, coords_k]
